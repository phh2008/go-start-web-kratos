...
type UserRepo interface {
    IBaseRepo
    ListPage(ctx context.Context, req model.UserListReq) (*model.PageData[*model.User], error)
...
// ListPage 用户列表
func (a *UserUseCase) ListPage(ctx context.Context, req model.UserListReq) *result.Result[*model.PageData[*model.User]] {
    data, err := a.userRepo.ListPage(ctx, req)
    if err != nil {
        return result.Error[*model.PageData[*model.User]](err)
    }
    return result.Ok(data)
}

// CreateByEmail 根据邮箱创建用户
func (a *UserUseCase) CreateByEmail(ctx context.Context, email model.UserEmailRegister) *result.Result[model.User] {
    user, err := a.userRepo.GetByEmail(ctx, email.Email)
    if err != nil {
        return result.Error[model.User](err)
    }
    if user.Id > 0 {
        return result.Failure[model.User]("email 已存在")
    }
    ...
    if err != nil {
        ...
        return result.Failure[model.User]("创建用户出错")
    }
    return result.Ok[model.User](*ret)
}
